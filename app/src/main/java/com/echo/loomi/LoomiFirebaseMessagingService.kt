...
class LoomiFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        try {
            ...
            if (data.isNotEmpty()) {
                ...
                if (type == "call") {
                    if (!isAppInForeground) {
                        ...
                        NotificationHelper.showCallNotification(applicationContext, callerId, callerName, callerImage)
                    }
                } else if (type == "screenshot") {
                    if (!isAppInForeground) {
                        val senderName = data["senderName"] ?: "Someone"
                        NotificationHelper.showMessageNotification(applicationContext, data["senderId"] ?: "system", senderName, "", "📷 $senderName took a screenshot!", data["chatId"] ?: "")
                    }
                } else if (type == "sos") {
                    val senderName = data["senderName"] ?: "Someone"
                    ...
                    NotificationHelper.showSOSNotification(applicationContext, senderName, email, battery, deviceModel, lat, lon, timestamp)
                } else {
                    if (!isAppInForeground) {
                        ...
                        if (senderId.isNotEmpty()) {
                            NotificationHelper.showMessageNotification(applicationContext, senderId, senderName, senderImage, messageText, chatId)
                        }
                    }
                }
            } else {
                if (!isAppInForeground) {
                    message.notification?.let { notification ->
                        NotificationHelper.showMessageNotification(applicationContext, "system", notification.title ?: "Loomi", "", notification.body ?: "", "system_chat")
                    }
                }
            }
        } catch (e: Exception) {
            ...
            if (wakeLock.isHeld) wakeLock.acquire(1) // dummy to avoid double release if needed
